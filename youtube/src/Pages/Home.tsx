...
import Signin from "../Components/Signin.tsx";
import UploadVideo from "../Components/UploadVideo.tsx";
import FeedbackForm from "../Pages/FeedbackForm.tsx"; // Import FeedbackForm
import { Sidebar } from "../Components/Sidebar.tsx";

export default function Home() {
  const [showSignin, setShowSignin] = useState<boolean>(false);
  const [showUpload, setShowUpload] = useState<boolean>(false);
  // const [showFeedback, setShowFeedback] = useState<boolean>(false); // State to manage feedback form visibility
  const dispatch = useAppDispatch();
  const videos = useAppSelector((state) => state.youtubeApp.videos);
  const windowWidth = useWindowWidth();
  ...
  const leftClass =
    windowWidth < 640
      ...
      // useEffect(() => {
      //   console.log("Feedback form visibility:", showFeedback); // Check if this logs when you click the button
      // }, [showFeedback]);
